...
class PatientTestDataSeederScreen extends StatefulWidget {
  ...
  @override
  State<PatientTestDataSeederScreen> createState() => _PatientTestDataSeederScreenState();
}

class _PatientTestDataSeederScreenState extends State<PatientTestDataSeederScreen> {
  ...
  Future<void> _loadAccounts() async {
    ...
    final AuthAccount? doctor = await AuthRegistry.instance.accountForUserId(_doctorIdController.text.trim());
    final AuthAccount? pharmacist = await AuthRegistry.instance.accountForUserId(_pharmacistIdController.text.trim());
    final AuthAccount? patient = await AuthRegistry.instance.accountForUserId(_patientIdController.text.trim());

    setState(() {
      _doctor = (doctor != null && doctor.role == AuthRole.doctor) ? doctor : null;
      _pharmacist = (pharmacist != null && pharmacist.role == AuthRole.pharmacist) ? pharmacist : null;
      _patient = (patient != null && patient.role == AuthRole.patient) ? patient : null;
      _loadingAccounts = false;
    });

    _appendLog('Doctor: ${_doctor == null ? "not found / wrong role" : "${_doctor!.fullName} (${_doctor!.userId})"}');
    _appendLog('Pharmacist: ${_pharmacist == null ? "not found / wrong role" : "${_pharmacist!.fullName} (${_pharmacist!.userId})"}');
    _appendLog('Patient: ${_patient == null ? "not found / wrong role" : "${_patient!.fullName} (${_patient!.userId})"}');
  }
  ...
  Future<void> _seedAvailability() async {
    try {
      ...
      final int otherDay = today == DateTime.monday ? DateTime.wednesday : DateTime.monday;

      await repo.create(doctorId: _doctor!.userId, weekday: today, startTime: '16:00', endTime: '18:00', capacity: 5);
      await repo.create(doctorId: _doctor!.userId, weekday: otherDay, startTime: '10:00', endTime: '12:00', capacity: 20);
  ...
  Future<void> _seedPrescriptions() async {
    try {
      ...
      await repo.create(
        ...
        additionalNotes: 'Seed test data — follow up in 1 week if symptoms persist.',
        medicines: const [
  ...
  Future<DoctorAvailabilityWindow?> _todaysWindow() async {
    const DoctorAvailabilityRepository repo = DoctorAvailabilityRepository();
    final List<DoctorAvailabilityWindow> windows = await repo.fetchForDoctor(_doctor!.userId);
    if (windows.isEmpty) return null;
    final int today = DateTime.now().weekday;
    return windows.firstWhere((w) => w.weekday == today, orElse: () => windows.first);
  }
  ...
  Future<void> _seedAppointment() async {
    try {
      ...
      _appendLog('Appointment: booked patient into ${window.weekdayName} ${window.label}');
    } catch (e) {
  ...
  Future<void> _fillWindowToCapacity() async {
    try {
      ...
      final int toCreate = (window.capacity - already).clamp(0, window.capacity);
      ...
      _appendLog('Fill to Capacity: added $toCreate filler booking(s) — window should now show Full');
    } catch (e) {
  ...
  Future<void> _resetProfile() async {
    try {
      await FirebaseFirestore.instance.collection('patient_profiles').doc(_patient!.userId).delete();
      _appendLog('Profile: deleted (simulates a fresh signup — onboarding will show on next login)');
    } catch (e) {
  ...
  Future<void> _seedDoseLogs() async {
    try {
      ...
      final List<PrescriptionRecord> prescriptions = await prescriptionRepo.fetchForPatient(_patient!.userId);
      ...
      final List<ScheduledDose> past = doses.where((d) => d.scheduledAt.isBefore(now)).toList();

      int marked = 0;
      for (int i = 0; i < past.length; i++) {
        if (i.isEven) {
          await doseLogRepo.markTaken(patientId: _patient!.userId, dose: past[i]);
          marked++;
        }
      }

      _appendLog('Dose Logs: marked $marked of ${past.length} past scheduled doses as taken');
    } catch (e) {
  ...
  Future<void> _seedPharmacyVisit() async {
    try {
      ...
      await visitService.markVisited(patientId: _patient!.userId, pharmacyId: pharmacyId);
  ...
  Future<void> _seedLabReport() async {
    try {
      ...
      final Uint8List bytes = Uint8List.fromList(img.encodeJpg(placeholder, quality: 70));
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Doctor userId', border: OutlineInputBorder(), isDense: true),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: _pharmacistIdController,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Pharmacist userId', border: OutlineInputBorder(), isDense: true),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: _patientIdController,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Patient userId', border: OutlineInputBorder(), isDense: true),
            ),
            const SizedBox(height: 10),
            ElevatedButton(
              onPressed: _loadingAccounts ? null : _loadAccounts,
              style: ElevatedButton.styleFrom(backgroundColor: MColors.primaryColor, foregroundColor: Colors.white),
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _SeedButton(label: 'Doctor Availability', enabled: _ready, onPressed: _seedAvailability),
                _SeedButton(label: 'Prescriptions', enabled: _ready, onPressed: _seedPrescriptions),
                _SeedButton(label: 'Appointment', enabled: _ready, onPressed: _seedAppointment),
                _SeedButton(label: 'Fill Window to Capacity', enabled: _ready, onPressed: _fillWindowToCapacity),
                _SeedButton(label: 'Patient Profile', enabled: _ready, onPressed: _seedProfile),
                _SeedButton(label: 'Reset Profile', enabled: _ready, onPressed: _resetProfile),
                _SeedButton(label: 'Dose Logs', enabled: _ready, onPressed: _seedDoseLogs),
                _SeedButton(label: 'Pharmacy Visit + Review', enabled: _ready, onPressed: _seedPharmacyVisit),
                _SeedButton(label: 'Lab Report', enabled: _ready, onPressed: _seedLabReport),
              ],
            ),
            const SizedBox(height: 16),
            const Divider(),
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, i) => Padding(
                  ...
                  child: Text(_log[i], style: Theme.of(context).textTheme.bodySmall),
                ),
              ),
            ),
          ],
...
class _SeedButton extends StatefulWidget {
  const _SeedButton({required this.label, required this.enabled, required this.onPressed});
