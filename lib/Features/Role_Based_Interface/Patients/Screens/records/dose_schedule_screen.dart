...
class _DoseScheduleScreenState extends State<DoseScheduleScreen> {
  ...
  List<String> get _doctorNames {
    ...
    for (final PrescriptionRecord p in widget.activePrescriptions) {
      if (p.doctorName.isNotEmpty && !names.contains(p.doctorName))
        names.add(p.doctorName);
    }
    return names;
  }
  ...
  Future<void> _load() async {
    ...
    try {
      final List<DoseLogEntry> logs = await _repository.fetchForPatient(
        widget.patientId,
      );
      if (!mounted) return;
      setState(() {
    ...
    return [
      for (int i = 6; i >= 0; i--)
        () {
          ...
          final List<ScheduledDose> doses = _dosesOn(
            day,
          ).where((d) => !d.scheduledAt.isAfter(DateTime.now())).toList();
          final int taken = doses
              .where((d) => _logFor(d)?.isTaken ?? false)
              .length;
  ...
  String _streakSentence(List<_DayAdherence> last7) {
    ...
    final Iterable<_DayAdherence> shortfalls = priorDays.where(
      (d) => d.hasShortfall,
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Couldn't update that dose — try again."),
          ),
        );
      }
    }
        ...
        child: _loading
            ...
            : Builder(
                builder: (context) {
                  ...
                  final int totalTaken = last7.fold(
                    0,
                    (sum, d) => sum + d.taken,
                  );
                  final int totalDue = last7.fold(0, (sum, d) => sum + d.due);
                  final double weekRatio = totalDue == 0
                      ? 1
                      : totalTaken / totalDue;
                  ...
                  final List<ScheduledDose> todaysDosesAll = _dosesOn(
                    todayOnly,
                    applyFilter: false,
                  );
                  final List<ScheduledDose> todaysDoses = _dosesOn(todayOnly);
                  final int todayTaken = todaysDoses
                      .where((d) => _logFor(d)?.isTaken ?? false)
                      .length;
                  final Set<String> overlapKeys = overlappingDoseKeys(
                    todaysDosesAll,
                  );
                  ...
                  return ListView(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Text(
                            'Today · $todayTaken/${todaysDoses.length} taken',
                            style: theme.textTheme.titleMedium,
                          ),
                        ],
                      ),
                      ...
                      if (todaysDoses.isEmpty)
                        ...
                        for (int i = 0; i < todaysDoses.length; i++) ...[
                          _DoseRow(
                            ...
                            isDark: isDark,
                            isOverlapping: overlapKeys.contains(
                              todaysDoses[i].overlapKey,
                            ),
                            onToggle: (currentlyTaken) =>
                                _toggleTaken(todaysDoses[i], currentlyTaken),
                          ),
                          if (i != todaysDoses.length - 1)
                      ...
                      child: FractionallySizedBox(
                        ...
                        child: Container(
                          color: isFull
                              ...
                              : MColors.primaryColor.withValues(alpha: 0.45),
                        ),
                      ),
                    ),
            ),
      ...
      children: [
        ...
        Material(
          ...
          shape: const CircleBorder(
            side: BorderSide(color: MColors.primaryColor, width: 1.5),
          ),
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Icon(
                Icons.check,
                size: 18,
                color: isTaken ? Colors.white : MColors.primaryColor,
              ),
            ),
          ),
        ),
