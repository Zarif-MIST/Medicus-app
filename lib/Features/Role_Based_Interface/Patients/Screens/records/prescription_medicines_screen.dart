...
class PrescriptionMedicinesScreen extends StatefulWidget {
  ...
  @override
  State<PrescriptionMedicinesScreen> createState() => _PrescriptionMedicinesScreenState();
}

class _PrescriptionMedicinesScreenState extends State<PrescriptionMedicinesScreen> {
  ...
  List<MapEntry<String, List<Prescription>>> _groupedByDoctor(List<Prescription> prescriptions) {
    final Map<String, List<Prescription>> byDoctor = {};
    for (final Prescription p in prescriptions) {
      (byDoctor[p.doctorName] ??= []).add(p);
    }
    final List<MapEntry<String, List<Prescription>>> entries = byDoctor.entries.toList()
            ...
            : Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Text('Sort by', style: theme.textTheme.bodySmall?.copyWith(color: Colors.grey)),
                        const SizedBox(width: 10),
                        _SortChip(
                          label: 'Date',
                          selected: _sortMode == _SortMode.byDate,
                          onTap: () => setState(() => _sortMode = _SortMode.byDate),
                        ),
                        const SizedBox(width: 8),
                        _SortChip(
                          label: 'Doctor',
                          selected: _sortMode == _SortMode.byDoctor,
                          onTap: () => setState(() => _sortMode = _SortMode.byDoctor),
                        ),
                      ],
                    ),
                  ),
                    ...
                    child: visible.isEmpty
                        ...
                        : _sortMode == _SortMode.byDate
                            ? ListView.separated(
                                ...
                                separatorBuilder: (_, _) => SizedBox(height: pad * 0.7),
                                itemBuilder: (context, index) => _PrescriptionGroup(
                                  prescription: visible[index],
                                  isDark: isDark,
                                  formattedDate: _formattedDate(visible[index].date),
                                ),
                              )
                            : _DoctorGroupedList(
...
class _PrescriptionGroup extends StatelessWidget {
  const _PrescriptionGroup({required this.prescription, required this.isDark, required this.formattedDate});
        ...
        children: [
          Row(
            children: [
              ...
              Text(formattedDate, style: theme.textTheme.bodySmall?.copyWith(color: Colors.grey)),
            ],
          ),
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          child: const Icon(Icons.medication_outlined, color: MColors.primaryColor, size: 18),
        ),
