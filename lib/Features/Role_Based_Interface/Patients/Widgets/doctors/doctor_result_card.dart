...
class DoctorSummary {
  ...
  factory DoctorSummary.fromAuthAccount(AuthAccount account) {
    ...
    return DoctorSummary(
      ...
      name: displayName,
      specialty: (account.specialty ?? '').trim().isEmpty
          ? 'General Physician'
          : account.specialty!.trim(),
      hospital: '',
      rating: 0,
      experienceYears: 0,
      fee: 0,
...
class DoctorResultCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Plain Material, not glass — the card fill below is fully opaque, so
    // the glass shader behind it was pure cost with no visible effect,
    // and repainting it on every scroll frame caused this list's cards
    // to blank out for a moment after a fling gesture.
    return Material(
