...
class ScheduledDose {
  ...
  String get logId {
    ...
    final String timeKey = '${scheduledAt.hour.toString().padLeft(2, '0')}${scheduledAt.minute.toString().padLeft(2, '0')}';
    return '${prescriptionId}_${medicineIndex}_${dateKey}_$timeKey';
  }

  bool isSameDayAs(DateTime other) =>
      scheduledAt.year == other.year && scheduledAt.month == other.month && scheduledAt.day == other.day;
  ...
  String get overlapKey => '${medicineName.trim().toLowerCase()}_${scheduledAt.hour}:${scheduledAt.minute}';
}
...
Set<String> overlappingDoseKeys(List<ScheduledDose> doses) {
  ...
  for (final ScheduledDose dose in doses) {
    (prescriptionIdsByKey[dose.overlapKey] ??= <String>{}).add(dose.prescriptionId);
  }
  return {
    for (final MapEntry<String, Set<String>> entry in prescriptionIdsByKey.entries)
...
List<ScheduledDose> expandDoseSchedule({
  ...
  for (final record in prescriptions) {
    ...
    for (int medicineIndex = 0; medicineIndex < record.medicines.length; medicineIndex++) {
      final PrescriptionRecordMedicine medicine = record.medicines[medicineIndex];
      if (medicine.doseTimes.isEmpty || medicine.durationDays <= 0) continue;

      final DateTime courseEnd = prescribedOn.add(Duration(days: medicine.durationDays - 1));
