...
class PrescriptionTimelineEntry {
  ...
  double get progress =>
      dayTotal == 0 ? 0 : (dayCurrent / dayTotal).clamp(0, 1);
}
...
class PrescriptionTimeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Set<String> overlapNames =
        overlapNamesOverride ?? overlappingMedicineNames(entries);
    return Column(
      children: [
        for (int i = 0; i < entries.length; i++)
          _PrescriptionTimelineRow(
            ...
            isOverlapping: overlapNames.contains(
              entries[i].medicineName.trim().toLowerCase(),
            ),
          ),
      ],
    );
...
class _PrescriptionTimelineRow extends StatelessWidget {
  const _PrescriptionTimelineRow({
    required this.entry,
  ...
  Widget build(BuildContext context) {
    ...
    final Color statusColor = entry.isCompleted
        ? Colors.green
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text(
                            entry.isCompleted
                                ? 'Completed'
                                : 'Day ${entry.dayCurrent}/${entry.dayTotal}',
